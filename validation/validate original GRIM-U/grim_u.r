...
simrank_optimised <- function(n1, n2, U_target, max_iter = 1000) {
  ...
  if (R1_target < min_sum || R1_target > max_sum) {
    return(NA)
  }
...
run_simulation_step <- function(k, n1, n2) {
  ...
  return(tibble(p_exact = m1$p.value, p_approx = m2$p.value))
}
...
plot_grimu_steps <- function(df_results) {
  ...
  ggplot(
    plot_data,
    aes(x = U_Values, y = p_value, color = Method, shape = Method)
  ) +
    ...
    labs(
      ...
      subtitle = paste0(
        "Visualizing the discrete steps in p-values for N1=",
        unique(df_results$n1),
        ", N2=",
        unique(df_results$n2)
      ),
